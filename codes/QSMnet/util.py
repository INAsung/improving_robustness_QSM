...
def compute_all_torch(im1, im2, mask):
    return compute_nrmse_torch(im1,im2,mask), compute_psnr_torch(im1,im2,mask), compute_ssim_torch(im1,im2,mask), compute_hfen_torch(im1,im2,mask)

def compute_nrmse_torch(im1, im2, mask):
    im1_masked = im1[mask]
    im2_masked = im2[mask]
    mse = torch.mean((im1_masked - im2_masked) ** 2)
    nrmse = torch.sqrt(mse) / torch.sqrt(torch.mean(im2_masked ** 2))
    return 100 * nrmse

def compute_psnr_torch(im1, im2, mask):
    im1_masked = im1[mask]
    im2_masked = im2[mask]
    mse = torch.mean((im1_masked - im2_masked) ** 2)
    if mse == 0:
...
def compute_ssim_torch(im1, im2, mask):
    im1 = torch.tensor(im1).float()
    im2 = torch.tensor(im2).float()
    mask = torch.tensor(mask).bool()

    im1 = F.pad(im1, (5, 5, 5, 5, 5, 5), 'constant', 0)
    im2 = F.pad(im2, (5, 5, 5, 5, 5, 5), 'constant', 0)
    mask = F.pad(mask, (5, 5, 5, 5, 5, 5), 'constant', 0)
    im1 = im1.clone()
    im2 = im2.clone()

    min_im = torch.min(torch.min(im1[mask], im2[mask]))
    im1[mask] = im1[mask] - min_im
    im2[mask] = im2[mask] - min_im

    max_im = torch.max(torch.max(im1[mask], im2[mask]))
    im1 = 255 * im1 / max_im
    im2 = 255 * im2 / max_im

    im1_np = im1.detach().cpu().numpy()
    im2_np = im2.detach().cpu().numpy()

    ssim_val = ssim(im1, im2, data_range=255, multichannel=True)
    return ssim_val

def compute_hfen_torch(im1, im2, mask):
    sigma = 1.5
    x, y, z = torch.meshgrid(torch.arange(-7, 8), torch.arange(-7, 8), torch.arange(-7, 8))
    h = torch.exp(-(x**2 + y**2 + z**2) / (2 * sigma**2))
    h = h / torch.sum(h)

    arg = (x**2 + y**2 + z**2) / (sigma**4) - (1 + 1 + 1) / (sigma**2)
    H = arg * h
    H = H - torch.sum(H) / (15**3)

    im1_np = im1.detach().cpu().numpy()
    im2_np = im2.detach().cpu().numpy()
    im1_log = scipy.ndimage.correlate(im1_np, H, mode='constant')
    im2_log = scipy.ndimage.correlate(im2_np, H, mode='constant')
    
    return compute_nrmse(im1_log, im2_log, mask.detach().cpu().numpy())