...
if 1:
    ...
    parser.add_argument('--TRAIN_LR',      type=float,  default=0.005)
...
if 2:
    ...
    args["save_dir"] = f'/fast_storage/intern1/codes/denoising_agents/2DUnet/save/{formatted_weights}'
...
transform = transforms.Compose([ToTensor(),RandomFlip(), Random_or_Center_Crop2D([144,144])])
transform_origin = transforms.Compose([ToTensor()])

dataset_train = Dataset('/fast_storage/intern1/QSMnet_data/noisepatch_2D_all.hdf5', transform=transform)
...
for epoch in range(args["TRAIN_EPOCH"]): 
    ...
    ## 1. train dataset
        ...
        # ssim loss
        ssim_out = ssim(output, clean, mask)
    ...
    with torch.no_grad():
        ...
        for batch, data in enumerate(loader_val):
            for std_now, noisy_each in enumerate(data['noisy']):
                ...
                noisy = noisy_each.to(device).squeeze(0)
                ...
                ssim_norm = ssim(clean, output, mask)
                ...
                ssim_ori = compute_ssim(clean_ori, output_ori,mask)
            ...
            if batch==0 and std_now == 9:
                ...
                dtimg.figsize = (12,8)
                dtimg.data.append(rotate(noisy_ori[sl,:,:,:].squeeze(1).squeeze(0).cpu().detach().numpy(), -90)); dtimg.v.append((-0.05,0.05)); dtimg.label.append('noisy_std9')
                dtimg.data.append(rotate(output_ori[sl,:,:,:].squeeze(1).squeeze(0).cpu().detach().numpy(), -90)); dtimg.v.append((-0.05,0.05));   dtimg.label.append('predicted_denoised')
                dtimg.data.append(rotate(clean_ori[sl,:,:,:].squeeze(1).squeeze(0).cpu().detach().numpy(), -90)); dtimg.v.append((-0.05,0.05));   dtimg.label.append('clean')
